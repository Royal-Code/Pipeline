...
namespace RoyalCode.Core.Cqs
{
        ...
        where TNextRequest : IRequest<TResult>
    {
        ...
        Task<TResult> NextAsync(TRequest request, Func<TNextRequest, CancellationToken, Task<TResult>> next, CancellationToken token = default);
    }
        ...
        where TNextRequest : IRequest
    {
        ...
        /// <param name="token">Token para cancelamento da tarefa.</param>
        /// <returns>A tarefa assíncrona da execução.</returns>
        Task NextAsync(TRequest request, Func<TNextRequest, CancellationToken, Task> next, CancellationToken token = default);
    }
        ...
        where TNextRequest : IRequest<TNextResult>
    {
        ...
        /// <param name="token">Token para cancelamento da tarefa.</param>
        /// <returns>A tarefa assíncrona da execução.</returns>
        Task<TResult> NextAsync(TRequest request, Func<TNextRequest, CancellationToken, Task<TNextResult>> next, CancellationToken token = default);
    }
}
