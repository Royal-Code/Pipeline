...
namespace RoyalCode.PipelineFlow.Configurations
{
    ...
    public static class PipelineBuilderExtensions
    {
        ...
        public static IPipelineBuilder<TIn> Handle<TIn>(
            this IPipelineBuilder<TIn> builder, Action<TIn> handler)
        {
            builder.AddHandlerResolver(DefaultHandlersResolver.Handle(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn> Handle<TService, TIn>(
            this IPipelineBuilderWithService<TService, TIn> builder,
            Action<TService, TIn> handler)
        {
            builder.AddHandlerResolver(DefaultHandlersResolver.Handle(handler));
            return builder.Configure<TIn>();
        }

        public static IPipelineBuilder<TIn> HandleAsync<TIn>(
            this IPipelineBuilder<TIn> builder, 
        ...
        public static IPipelineBuilder<TIn> HandleAsync<TIn>(
            this IPipelineBuilder<TIn> builder,
            Func<TIn, CancellationToken, Task> handler)
        {
            builder.AddHandlerResolver(DefaultHandlersResolver.HandleAsync(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn> HandleAsync<TService, TIn>(
            this IPipelineBuilderWithService<TService, TIn> builder, 
            Func<TService, TIn, Task> handler)
        {
            builder.AddHandlerResolver(DefaultHandlersResolver.HandleAsync(handler));
            return builder.Configure<TIn>();
        }
        ...
        public static IPipelineBuilder<TIn> HandleAsync<TService, TIn>(
            this IPipelineBuilderWithService<TService, TIn> builder, 
            Func<TService, TIn, CancellationToken, Task> handler)
        {
            builder.AddHandlerResolver(DefaultHandlersResolver.HandleAsync(handler));
            return builder.Configure<TIn>();
        }
        ...
        public static IPipelineBuilder<TIn, TOut> Handle<TIn, TOut>(
            this IPipelineBuilder<TIn, TOut> builder, 
        ...
        public static IPipelineBuilder<TIn, TOut> Handle<TService, TIn, TOut>(
            this IPipelineBuilderWithService<TService, TIn, TOut> builder,
            Func<TService, TIn, TOut> handler)
        {
            builder.AddHandlerResolver(DefaultHandlersResolver.Handle(handler));
            return builder.Configure<TIn, TOut>();
        }

        public static IPipelineBuilder<TIn, TOut> HandleAsync<TIn, TOut>(
            this IPipelineBuilder<TIn, TOut> builder, 
        ...
        public static IPipelineBuilder<TIn, TOut> HandleAsync<TService, TIn, TOut>(
            this IPipelineBuilderWithService<TService, TIn, TOut> builder,
            Func<TService, TIn, Task<TOut>> handler)
        {
            builder.AddHandlerResolver(DefaultHandlersResolver.HandleAsync(handler));
            return builder.Configure<TIn, TOut>();
        }
        ...
        public static IPipelineBuilder<TIn, TOut> HandleAsync<TService, TIn, TOut>(
            this IPipelineBuilderWithService<TService, TIn, TOut> builder,
            Func<TService, TIn, CancellationToken, Task<TOut>> handler)
        {
            builder.AddHandlerResolver(DefaultHandlersResolver.HandleAsync(handler));
            return builder.Configure<TIn, TOut>();
        }
        ...
        public static IPipelineBuilder<TIn> BridgeHandle<TIn, TNextInput>(
            this IPipelineBuilder<TIn> builder, Action<TIn,
                Action<TNextInput>> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandler(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn> BridgeHandleAsync<TIn, TNextInput>(
            this IPipelineBuilder<TIn> builder,
            Func<TIn, Func<TNextInput, Task>, Task> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn> BridgeHandleAsync<TIn, TNextInput>(
            this IPipelineBuilder<TIn> builder,
            Func<TIn, Func<TNextInput, Task>, CancellationToken, Task> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn> BridgeHandle<TService, TIn, TNextInput>(
            this IPipelineBuilderWithService<TService, TIn> builder,
            Action<TService, TIn, Action<TNextInput>> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandler(handler));
            return builder.Configure<TIn>();
        }

        public static IPipelineBuilder<TIn> BridgeHandleAsync<TService, TIn, TNextInput>(
            this IPipelineBuilderWithService<TService, TIn> builder, 
            Func<TService, TIn, Func<TNextInput, Task>, Task> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync(handler));
            return builder.Configure<TIn>();
        }
        ...
        public static IPipelineBuilder<TIn> BridgeHandleAsync<TService, TIn, TNextInput>(
            this IPipelineBuilderWithService<TService, TIn> builder,
            Func<TService, TIn, Func<TNextInput, Task>, CancellationToken, Task> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync(handler));
            return builder.Configure<TIn>();
        }
        ...
        public static IPipelineBuilder<TIn, TOut> BridgeHandle<TIn, TOut, TNextInput>(
            this IPipelineBuilder<TIn, TOut> builder,
            Func<TIn, Func<TNextInput, TOut>, TOut> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandler<TIn, TOut, TNextInput>(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn, TOut> BridgeHandleAsync<TIn, TOut, TNextInput>(
            this IPipelineBuilder<TIn, TOut> builder,
            Func<TIn, Func<TNextInput, Task<TOut>>, Task<TOut>> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync<TIn, TOut, TNextInput>(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn, TOut> BridgeHandleAsync<TIn, TOut, TNextInput>(
            this IPipelineBuilder<TIn, TOut> builder,
            Func<TIn, Func<TNextInput, Task<TOut>>, CancellationToken, Task<TOut>> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync<TIn, TOut, TNextInput>(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn, TOut> BridgeHandle<TService, TIn, TOut, TNextInput>(
            this IPipelineBuilderWithService<TService, TIn, TOut> builder,
            Func<TService, TIn, Func<TNextInput, TOut>, TOut> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandler<TService, TIn, TOut, TNextInput>(handler));
            return builder.Configure<TIn, TOut>();
        }

        public static IPipelineBuilder<TIn, TOut> BridgeHandleAsync<TService, TIn, TOut, TNextInput>(
            this IPipelineBuilderWithService<TService, TIn, TOut> builder, 
            Func<TService, TIn, Func<TNextInput, Task<TOut>>, Task<TOut>> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync<TService, TIn, TOut, TNextInput>(handler));
            return builder.Configure<TIn, TOut>();
        }

        public static IPipelineBuilder<TIn, TOut> BridgeHandleAsync<TService, TIn, TOut, TNextInput>(
            this IPipelineBuilderWithService<TService, TIn, TOut> builder,
            Func<TService, TIn, Func<TNextInput, Task<TOut>>, CancellationToken, Task<TOut>> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync<TService, TIn, TOut, TNextInput>(handler));
            return builder.Configure<TIn, TOut>();
        }
        ...
        public static IPipelineBuilder<TIn, TOut> BridgeHandle<TIn, TOut, TNextInput, TNextOutput>(
            this IPipelineBuilder<TIn, TOut> builder,
            Func<TIn, Func<TNextInput, TNextOutput>, TOut> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandler(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn, TOut> BridgeHandleAsync<TIn, TOut, TNextInput, TNextOutput>(
            this IPipelineBuilder<TIn, TOut> builder,
            Func<TIn, Func<TNextInput, Task<TNextOutput>>, Task<TOut>> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn, TOut> BridgeHandleAsync<TIn, TOut, TNextInput, TNextOutput>(this IPipelineBuilder<TIn, TOut> builder, Func<TIn, Func<TNextInput, Task<TNextOutput>>, CancellationToken, Task<TOut>> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync(handler));
            return builder;
        }

        public static IPipelineBuilder<TIn, TOut> BridgeHandle<TService, TIn, TOut, TNextInput, TNextOutput>(this IPipelineBuilderWithService<TService, TIn, TOut> builder, Func<TService, TIn, Func<TNextInput, TNextOutput>, TOut> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandler<TService, TIn, TOut, TNextInput, TNextOutput>(handler));
            return builder.Configure<TIn, TOut>();
        }

        public static IPipelineBuilder<TIn, TOut> BridgeHandleAsync<TService, TIn, TOut, TNextInput, TNextOutput>(this IPipelineBuilderWithService<TService, TIn, TOut> builder, Func<TService, TIn, Func<TNextInput, Task<TNextOutput>>, Task<TOut>> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync<TService, TIn, TOut, TNextInput, TNextOutput>(handler));
            return builder.Configure<TIn, TOut>();
        }

        public static IPipelineBuilder<TIn, TOut> BridgeHandleAsync<TService, TIn, TOut, TNextInput, TNextOutput>(this IPipelineBuilderWithService<TService, TIn, TOut> builder, Func<TService, TIn, Func<TNextInput, Task<TNextOutput>>, CancellationToken, Task<TOut>> handler)
        {
            builder.AddHandlerResolver(DefaultBridgeResolver.BridgeHandlerAsync<TService, TIn, TOut, TNextInput, TNextOutput>(handler));
            return builder.Configure<TIn, TOut>();
        }
        ...
        public static IPipelineBuilder<TInput> Decorate<TInput>(
            this IPipelineBuilder<TInput> builder,
            Action<TInput, Action> handler)
        {
            builder.AddDecoratorResolver(DefaultDecoratorsResolver.Decorate(handler));
            return builder;
        }

        public static IPipelineBuilder<TInput> DecorateAsync<TInput>(
            this IPipelineBuilder<TInput> builder, 
            Func<TInput, Func<Task>, Task> handler)
        {
        ...
        public static IPipelineBuilder<TInput> DecorateAsync<TInput>(
            this IPipelineBuilder<TInput> builder, 
            Func<TInput, Func<Task>, CancellationToken, Task> handler)
        {
            builder.AddDecoratorResolver(DefaultDecoratorsResolver.DecorateAsync(handler));
            return builder;
        }
        ...
        public static IPipelineBuilder<TInput> Decorate<TService, TInput>(
            this IPipelineBuilderWithService<TService, TInput> builder,
            Action<TService, TInput, Action> handler)
        {
            builder.AddDecoratorResolver(DefaultDecoratorsResolver.Decorate(handler));
        ...
        public static IPipelineBuilder<TInput> DecorateAsync<TService, TInput>(
            this IPipelineBuilderWithService<TService, TInput> builder,
            Func<TService, TInput, Func<Task>, Task> handler)
        {
            builder.AddDecoratorResolver(DefaultDecoratorsResolver.DecorateAsync(handler));
            return builder.Configure<TInput>();
        }
        ...
        public static IPipelineBuilder<TInput, TOutput> Decorate<TInput, TOutput>(
            this IPipelineBuilder<TInput, TOutput> builder, 
            Func<TInput, Func<TOutput>, TOutput> handler)
        {
            builder.AddDecoratorResolver(DefaultDecoratorsResolver.Decorate(handler));
            return builder;
        }

        public static IPipelineBuilder<TInput, TOutput> DecorateAsync<TInput, TOutput>(
            this IPipelineBuilder<TInput, TOutput> builder, 
            Func<TInput, Func<Task<TOutput>>, Task<TOutput>> handler)
        {
            builder.AddDecoratorResolver(DefaultDecoratorsResolver.DecorateAsync(handler));
            return builder;
        }
        ...
        public static IPipelineBuilder<TInput, TOutput> DecorateAsync<TInput, TOutput>(
            this IPipelineBuilder<TInput, TOutput> builder, Func<TInput, 
                Func<Task<TOutput>>, CancellationToken, Task<TOutput>> handler)
        {
            builder.AddDecoratorResolver(DefaultDecoratorsResolver.DecorateAsync(handler));
            return builder;
        }
        ...
        public static IPipelineBuilder<TInput, TOutput> Decorate<TService, TInput, TOutput>(
            this IPipelineBuilderWithService<TService, TInput, TOutput> builder,
            Func<TService, TInput, Func<TOutput>, TOutput> handler)
        {
            builder.AddDecoratorResolver(DefaultDecoratorsResolver.Decorate(handler));
            return builder.Configure<TInput, TOutput>();
        }

        public static IPipelineBuilder<TInput, TOutput> DecorateAsync<TService, TInput, TOutput>(
            this IPipelineBuilderWithService<TService, TInput, TOutput> builder, 
            Func<TService, TInput, Func<Task<TOutput>>, Task<TOutput>> handler)
        {
            builder.AddDecoratorResolver(DefaultDecoratorsResolver.DecorateAsync(handler));
            return builder.Configure<TInput, TOutput>();
        }
        ...
        public static IPipelineBuilder<TInput, TOutput> DecorateAsync<TService, TInput, TOutput>(
            this IPipelineBuilderWithService<TService, TInput, TOutput> builder, 
            Func<TService, TInput, Func<Task<TOutput>>, CancellationToken, Task<TOutput>> handler)
        {
            builder.AddDecoratorResolver(DefaultDecoratorsResolver.DecorateAsync(handler));
            return builder.Configure<TInput, TOutput>();
        }
